...
namespace _27_FrontToBackSqlConnection.Utilities.Extension
{
    public static class FileValidator
    {
        ...
        public static async Task<string> CreateFile(this IFormFile file, params string[] roots)
        {
            ...
            for (int i = 0; i < roots.Length; i++) path = Path.Combine(path, roots[i]);
            path = Path.Combine(path, fileName);
            using (FileStream fileStream = new FileStream(path, FileMode.Create)) await file.CopyToAsync(fileStream);
            return fileName;
        }
